...
PRIVATE void
#ifdef __STDC__
strings(struct DATA *p, char *tmp, int option)
#else
strings(p, tmp, option)
struct DATA *p;
char *tmp;
int option;
#endif
{
  ...
  if (option & 2)
  {
	if (tmp[0] && tmp[1])	/* Not a basic latin unicode string, treat as normal ascii */
	{
		option = 0;
    	i = strlen(tmp);
	}
	...
	{
		for (i = 0; (tmp[2*i] != 0) || (tmp[2*i+1] != 0); i++)
			;
	}
  }
  else
  ...
  while (i-- > 0) { /* put the sting */
    if ((option & 1) && !isprint((unsigned char)(*tmp)))
	{
      char tmp1 = n2h[((*tmp) >> 4 ) & 0x0f];
      char tmp2 = n2h[((*tmp)      ) & 0x0f];
	  PUT_CHAR('\\', p);
	  PUT_CHAR('x', p);
	  PUT_CHAR(tmp1, p);
	  PUT_CHAR(tmp2, p);
    ...
    if ( *data.pf == '%' ) { /* we got a magic % cookie */
      ...
      for (state = 1; *data.pf && state;) {
        switch (*(++data.pf)) {
          ...
          case 's':  /* string */
            STAR_ARGS(&data);
            strings(&data, va_arg(args, char *), 0x00);
            state = 0;
            break;
          case 'S':  /* string */
            STAR_ARGS(&data);
            strings(&data, va_arg(args, char *), 0x01);
            state = 0;
            break;
          case 't':  /* string */
            STAR_ARGS(&data);
            strings(&data, va_arg(args, char *), 0x02);
            state = 0;
            break;
          case 'T':  /* string */
            STAR_ARGS(&data);
...
#define OCT_S "\x10\x11""Octet Str !"
/* strings precisions */
...
/* other flags */
  ...
  printf("%%*.*d --> 10.10\n");
  snprintf(holder, sizeof holder, "[%*.*x]\n", 10, 10, 0xbb);
  printf("[%*.*x]\n", 10, 10, 0xbb);
  printf("%s\n", holder);

  printf("%%*.*c --> 10.10\n");
  snprintf(holder, sizeof holder, "[%*.*c]\n", 10, 10, 'A');
  printf("[%*.*c]\n", 10, 10, 'A');
  printf("%s\n", holder);
